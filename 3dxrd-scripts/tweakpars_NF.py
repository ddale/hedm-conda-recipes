...
for current_iteration in range(iterations):
	...
	print 'Prior value:', '%.4f' % p_opt_array[i]
        ## added by mkak 11.08.2014 (oringally 21.07.2014)
        ## check to make sure the change isn't too big
        if abs(p_opt_array[i]-p_opt[1])/abs(p_opt[1]) < 0.1: # 0.1 is 10 percent change
            #print 'Updating...'
            #print  abs(p_opt_array[i]-p_opt[1])/p_opt[1], abs(p_opt_array[i]-p_opt[1]),p_opt_array[i]-p_opt[1],p_opt_array[i],p_opt[1]
            #print 'error check over'
            p_opt_array[i] = p_opt[1]
        else:
            #print 'NOT UPDATED. Value too different from original value: %.4f' % p_opt_array[i]
	    p_opt_array[i] = p_opt[1]
        ##p_opt_array[i] = p_opt[1]
        r.parameterobj.parameters[name] = p_opt_array[i]
        print
